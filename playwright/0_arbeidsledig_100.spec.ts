...
test.describe('Tester arbeidsledigsøknad', () => {
  ...
  test('Søknad FRISKMELDT', async () => {
    ...
    await expect(
      sharedPage.getByText('Fra hvilken dato trengte du ikke lenger sykmeldingen?'),
    ).toBeVisible();
  ...
  test('Søknad ANDRE_INNTEKTSKILDER', async () => {
    ...
    await expect(
      sharedPage.locator('.undersporsmal .navds-checkbox label[for="687404"]'),
  ...
  test('Søknad OPPHOLD_UTENFOR_EOS', async () => {
    ...
    await expect(
      sharedPage.getByText(
        'Var du på reise utenfor EU/EØS mens du var sykmeldt 1. - 24. april 2020?',
      ),
    ).toBeVisible();
    await sharedPage.locator('[data-cy="ja-nei-stor"] input[value="JA"]').check();
  ...
  test('Søknad TIL_SLUTT', async () => {
    ...
    await expect(
      inntektskilder.locator('div', { hasText: 'Hvilke inntektskilder har du hatt?' }),
    ).toContainText('andre arbeidsforhold');
    await expect(
      inntektskilder.locator('div', { hasText: 'Er du sykmeldt fra dette?' }),
    ).toContainText('Ja');
