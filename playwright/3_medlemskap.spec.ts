...
test.describe('Søknad med alle opprinnelige spørsmål om medlemskap', () => {
    ...
    test('Gjennomfører hele søknadsflyten for medlemskap', async ({ page }) => {
        ...
        await test.step('Arbeid utenfor Norge', async () => {
            await expect(page.getByText('Arbeid utenfor Norge')).toBeVisible()
        ...
        await test.step('Opphold utenfor Norge', async () => {
            await expect(page.getByText('Opphold utenfor Norge')).toBeVisible()
            await svarJaHovedsporsmal(page)
        ...
        await test.step('Opphold utenfor EØS', async () => {
            await expect(page.getByText('Opphold utenfor EU/EØS')).toBeVisible()
            await svarJaHovedsporsmal(page)
            await svarCombobox(page, 'I hvilket land utenfor EU/EØS eller Sveits har du oppholdt deg?', 'Fra', 'Fransk Polynesia')
            ...
            await expect(page.getByRole('combobox', { name: 'I hvilket land utenfor EU/EØS eller Sveits har du oppholdt deg?' })).toHaveCount(2)
            await page.getByRole('combobox', { name: 'I hvilket land utenfor EU/EØS eller Sveits har du oppholdt deg?' }).last().type('Ba')
            ...
            await page.getByRole('group', { name: 'Hva gjorde du i utlandet?' }).last().getByRole('radio', { name: 'Jeg bodde der' }).check()
        ...
        await test.step('Oppholdstillatelse', async () => {
            await expect(page.getByText('Oppholdstillatelse')).toBeVisible()
            await svarJaHovedsporsmal(page)
        ...
        await test.step('Søknad TIL_SLUTT (oppsummering)', async () => {
            ...
            await sporsmalOgSvar(oppsummeringContainer, 'Har du arbeidet utenfor Norge i løpet av de siste 12 månedene før du ble syk?', 'Ja')
            ...
            const oppholdstillatelseAnswer = oppsummeringContainer.getByText('Er oppholdstillatelsen midlertidig eller permanent?').locator('xpath=following-sibling::*')
...
test.describe('Søknad med nytt spørsmål om oppholdstillatelse og kjent permanent oppholdstillatelse', () => {
    ...
    test('Gjennomfører søknadsflyten med permanent oppholdstillatelse', async ({ page }) => {
        ...
        await test.step('Har kjent permanent oppholdstillatelse', async () => {
            await expect(page.getByText('Oppholdstillatelse')).toBeVisible()
            await expect(page.getByText('Vi har mottatt denne oppholdstillatelsen fra Utlendingsdirektoratet:')).toBeVisible()
            ...
            await expect(page.getByText('Har Utlendingsdirektoratet gitt deg en oppholdstillatelse før 1. mai 2024?')).toBeVisible()
        ...
        await test.step('Søknad TIL_SLUTT (oppsummering)', async () => {
            ...
            const oppsummeringContainer = page.locator('[data-cy="oppsummering-fra-søknaden"]')
            await sporsmalOgSvar(oppsummeringContainer, 'Har Utlendingsdirektoratet gitt deg en oppholdstillatelse før 1. mai 2024?', 'Ja')
            await sporsmalOgSvar(oppsummeringContainer, 'Hvilken dato fikk du denne oppholdstillatelsen?', '01.')
            await sporsmalOgSvar(oppsummeringContainer, 'Hvilken periode gjelder denne oppholdstillatelsen?', '10. – 15.')
        })
    })
})
