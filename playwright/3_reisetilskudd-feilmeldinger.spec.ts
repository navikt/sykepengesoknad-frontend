...
test.describe('Tester feilmeldinger i reisetilskudd', () => {
    ...
    test('Ugyldig valg', async ({ page }) => {
        ...
        await page.locator('.navds-modal').getByText('Bekreft').click()
        await expect(page.locator('[data-cy="opplasting-form"]').getByText('Du må velge transportmiddel')).not.toBeVisible()
    })
    ...
    test('Negative beløp', async ({ page }) => {
        ...
        await page.locator('.navds-modal').getByText('Bekreft').click()
        await expect(page.locator('[data-cy="opplasting-form"]').getByText('Beløp kan ikke være negativt')).toBeVisible()
    })
    ...
    test('Høyere beløp enn maks', async ({ page }) => {
        ...
        await page.locator('.navds-modal').getByText('Bekreft').click()
        await expect(page.locator('[data-cy="opplasting-form"]').getByText('Beløp kan ikke være større enn 1 000 000')).toBeVisible()
    })
    ...
    test('Kan ikke skrive inn med 3 desimaler', async ({ page }) => {
        ...
        const inputValue = await page.locator('input[name=belop_input]').inputValue()
        expect(inputValue).toBe('100.25')
    })
    ...
    test('Gyldig beløp med 2 desimaler', async ({ page }) => {
        ...
        await page.locator('.navds-modal').getByText('Bekreft').click()
        await expect(page.locator('[data-cy="opplasting-form"]').getByText('Beløp kan ikke være større enn 10 000')).not.toBeVisible()
    })
    ...
    test('Gyldig beløp uten desimaler', async ({ page }) => {
        ...
        await page.locator('.navds-modal').getByText('Bekreft').click()
        await expect(page.locator('[data-cy="opplasting-form"]').getByText('Beløp kan ikke være större enn 10 000')).not.toBeVisible()
    })
    ...
    test('Legger inn taxi kvittering', async ({ page }) => {
        ...
        await page.locator('[data-cy="filopplasteren"] input[type=file]').setInputFiles('./cypress/fixtures/kvittering.pdf')
        ...
        await page.locator('[data-cy="filopplasteren"] input[type=file]').setInputFiles('./cypress/fixtures/kvittering.jpg')
        // Then try to upload second file to trigger multiple files error
        await page.locator('[data-cy="filopplasteren"] input[type=file]').setInputFiles('./cypress/fixtures/kvittering2.jpg')
        await page.getByRole('button', { name: 'Bekreft' }).click()
        ...
        await page.locator('[data-cy="filopplasteren"] input[type=file]').setInputFiles('./cypress/fixtures/kvittering.jpg')
        await page.getByRole('button', { name: 'Bekreft' }).click()
        await expect(page.locator('.navds-modal').getByText('Du kan ikke laste opp mer enn en fil')).not.toBeVisible()
    })
