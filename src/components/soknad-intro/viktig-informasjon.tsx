...
export const ViktigInformasjon = () => {
    ...
    const merknadsTyperSomVarsles = [
        'UGYLDIG_TILBAKEDATERING',
        'TILBAKEDATERING_KREVER_FLERE_OPPLYSNINGER',
    ]

    if (
        valgtSoknad.merknaderFraSykmelding?.some((a) =>
            merknadsTyperSomVarsles.includes(a.type)
        )
    ) {
        return (
            <div className="viktig-informasjon">
                <GuidePanel poster illustration={VeilederSVG}>
                    ...
                    <BodyLong spacing>
                        {tekst('viktig-informasjon.avsnitt.1')}
                    </BodyLong>
                    <BodyLong spacing>
                        {tekst('viktig-informasjon.avsnitt.2')}
                    </BodyLong>
                    <Label as="h3">
                        {tekst('viktig-informasjon.under-overskrift')}
                    </Label>
                    <BodyLong spacing>
                        {tekst('viktig-informasjon.avsnitt.3')}
                    </BodyLong>
                </GuidePanel>
            </div>
        )
