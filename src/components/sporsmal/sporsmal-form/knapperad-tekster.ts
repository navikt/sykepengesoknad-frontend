const KnapperadTekster = {

    'sykepengesoknad.ga-videre': 'Gå videre',
    'sykepengesoknad.send': 'Send søknaden',
    'sykepengesoknad.avbryt.simpel': 'Avbryt søknad',
    'sykepengesoknad.avbryt.sporsmal.forklaring': 'Du er i ferd med å avbryte søknaden. Du vil kunne gjenåpne søknaden igjen og fylle den ut igjen hvis du ønsker det.',
    'sykepengesoknad.avbryt.sporsmal': 'Er du sikker på at du ikke ønsker å bruke denne søknaden?',
    'sykepengesoknad.avbryt.ja': 'Ja, avbryt søknaden',
    'sykepengesoknad.avbryt.angre': 'Nei, jeg vil ikke avbryte søknaden likevel',
    'sykepengesoknad.avbryt.feilet': 'Beklager, søknaden kunne ikke avbrytes',

}

export default KnapperadTekster
