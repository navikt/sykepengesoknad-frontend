...
const CheckboxKomp = ({ sporsmal }: SpmProps) => {
    ...
    const andreArbeidsforhold = sporsmal.undersporsmal.filter((undersporsmal) => {
        undersporsmal.tag.startsWith('INNTEKTSKILDER_GHOST')
    })
    const sporsmals = sporsmal.undersporsmal.slice(andreArbeidsforhold.length)
    const komplettListe = andreArbeidsforhold.concat(sporsmals)
                            ...
                            <>
                                <ReadMore header={'Finner du ikke riktig alternativ?'}>
                                    <BodyShort className={'my-4'}>
                                        Da kan det være at det ikke er pensjonsgivende inntekt. Pensjonsgivende inntekt
                                        er som regel lønn eller betaling for arbeid du har utført, og som det betales
                                        skatt av. Det er bare slik inntekt som kan gi rett til sykepenger.
                                    </BodyShort>
                                    <BodyShort className={'my-4'}>
