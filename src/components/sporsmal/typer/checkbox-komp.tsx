...
const ForklaringAvValgtCheckbox = (svaralternativ: Sporsmal) => {
    ...
    if (svaralternativ.tag === TagTyper.INNTEKTSKILDE_SELVSTENDIG) {
        return (
            <BodyShort>
                ...
                leverer skattemelding for næringsdrivende, og fakturerer kunder og (ofte) lever av overskuddet. Du er
                følgelig din egen sjef, og ikke ansatt av andre i et arbeidsforhold.
            </BodyShort>
        )
    }
    ...
    if (svaralternativ.tag === TagTyper.INNTEKTSKILDE_FRILANSER) {
        return (
            <BodyShort>
                Du jobber frilans når du jobber på selvstendig basis for forskjellige kunder uten å være fast ansatt.
            </BodyShort>
        )
    }
    ...
    if (svaralternativ.tag === TagTyper.INNTEKTSKILDE_FOSTERHJEM) {
        return (
            <BodyShort>
                Å motta fosterhjemsgodtgjørelse betyr at du mottar penger for å ha omsorg for et barn som ikke kan bo
                hos foreldrene sine.
            </BodyShort>
        )
    }
    ...
    if (svaralternativ.tag === TagTyper.INNTEKTSKILDE_SELVSTENDIG_DAGMAMMA) {
        return (
            <BodyShort>
                Dette betyr at du jobber som dagmamma og har regelmessig ansvar for andres barn mot betaling på dagtid.
                En dagmamma har som regel ansvar for flere barn samtidig, enten hjemme hos seg selv eller hjemme hos ett
                av barna.
            </BodyShort>
        )
    }
