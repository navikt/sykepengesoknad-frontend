...
export const førsteNyttårsdag = (inputYear: number) => { return  dayjs(`${inputYear}-1-1`) } // New Year's Day
export const arbeidernesDag = (inputYear: number) => { return dayjs(`${inputYear}-5-1`) } // labor day
export const grunnlovsdagen = (inputYear: number) => { return dayjs(`${inputYear}-5-17`) } // constitution day
export const kristiHimmelfartsdag = (inputYear: number) => { return easterSunday(inputYear).add(39, 'days') } // ascension day
export const førstePinsedag = (inputYear: number) => { return easterSunday(inputYear).add(49, 'days') } // white sunday
export const andrePinsedag = (inputYear: number) => { return easterSunday(inputYear).add(50, 'days') } // white monday
export const førsteJuledag = (inputYear: number) => { return dayjs(`${inputYear}-12-25`) } // Christmas day
export const andreJuledag = (inputYear: number) => { return dayjs(`${inputYear}-12-26`) } // boxing day
...
export function rodeUkeDagerIPerioden(min?: Date, max?: Date) {
    ...
    roodeDager.forEach((dag) => {
        if ((start.isSame(dag, 'day') || slutt.isSame(dag, 'day') || (start.isBefore(dag) && slutt.isAfter(dag) )) && dag.day() !== 0 && dag.day() !== 6 ) {
