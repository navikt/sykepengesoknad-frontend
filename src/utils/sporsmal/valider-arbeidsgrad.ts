...
const useValiderArbeidsgrad = (sporsmal: Sporsmal) => {
    ...
    if (
        ...
        (sporsmal.tag !== TagTyper.HVOR_MYE_TIMER_VERDI &&
            sporsmal.tag !== TagTyper.HVOR_MYE_HAR_DU_JOBBET)
    ) {
        return { undefined }
    }
    ...
    const periodeDager = ukeDatoListe(
        periode.fom.toString(),
        periode.tom.toString()
    )
    ...
    const sykedagerForArbeidstakere = () => {
        const feriedager = hentPeriodeListe(
            hentSporsmal(valgtSoknad, TagTyper.FERIE_NAR_V2)!
        ).flatMap((periode) => ukeDatoListe(periode.fom, periode.tom))
        const permisjonsdager = hentPeriodeListe(
            hentSporsmal(valgtSoknad, TagTyper.PERMISJON_NAR_V2)!
        ).flatMap((periode) => ukeDatoListe(periode.fom, periode.tom))
        ...
        return periodeDager
            .filter(
                (dag) =>
                    !ekskluderteDager.find(
                        (ekskludertDag) =>
                            ekskludertDag.toString() === dag.toString()
                    )
            )
            .filter((dag) => dag.day() !== 0 && dag.day() !== 6)
            .filter((dag) => {
    ...
    const validerGrad = () => {
        ...
        return faktiskArbeidsGrad < forventetArbeidsGrad
            ? getLedetekst(
                  tekst('soknad.feilmelding.MINDRE_TIMER_ENN_FORVENTET'),
    ...
    const beregnGrad = () => {
        ...
        const timerPerUkeId = hentUndersporsmal(
            hovedSporsmal!,
            TagTyper.HVOR_MANGE_TIMER_PER_UKE
        )!.id
        const faktiskTimerId = hentUndersporsmal(
            hovedSporsmal!,
